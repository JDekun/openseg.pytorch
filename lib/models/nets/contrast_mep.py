...
class MLM_Module(nn.Module):
    def __init__(self, 
                 configer,
                 inout_dim,
                 ):
        super(MLM_Module, self).__init__()
        ...
        #         self.de_projector_layer4 = nn.Sequential(
        #             nn.Conv2d(self.proj_dim, inout_dim, kernel_size=1, stride=1, padding=0, bias=True),
        #             ModuleHelper.BatchNorm2d(bn_type=self.configer.get("network", "bn_type"))(inout_dim),)
        #     elif layer == "layer_3":
        #         self.de_projector_layer3 = nn.Sequential(
        #             nn.Conv2d(self.proj_dim, inout_dim, kernel_size=1, stride=1, padding=0, bias=True),
        #             ModuleHelper.BatchNorm2d(bn_type=self.configer.get("network", "bn_type"))(inout_dim),)
    ...
    def forward(self, feat3, feat4, feat5):
        output = dict()
        layer = dict()
        # origin_feat3, origin_feat4, origin_feat5 = feat3, feat4, feat5

        for lay in self.projector:
            if lay == 'layer_4':
                proj_layer4 = F.normalize(self.projector_layer4(feat5), dim=1)
                layer['layer_4'] = proj_layer4
                # feat5 = self.relu(proj_layer4 + origin_feat5)
            elif lay == 'layer_3':
                proj_layer3 = F.normalize(self.projector_layer3(feat4), dim=1)
                layer['layer_3'] = proj_layer3
                # feat4 = self.relu(proj_layer3 + origin_feat4)
            elif lay == 'layer_2':
                proj_layer2 = F.normalize(self.projector_layer2(feat3), dim=1)
                layer['layer_2'] = proj_layer2
                # feat3 = self.relu(proj_layer2 + origin_feat3)
            # elif lay == 'layer_1':
            #     proj_layer1 = F.normalize(self.projector_layer1(feat2), dim=1)
            #     layer['layer_1'] = proj_layer1
            #     feat2 = self.relu(self.de_projector_layer1(proj_layer1) + origin_feat2)
