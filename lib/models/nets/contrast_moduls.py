...
class SpatialGather_Module(nn.Module):
    ...
    def forward(self, feats, probs, gt_probs=None):
        if self.use_gt and gt_probs is not None:
            ...
            ocr_context = torch.matmul(probs, feats).permute(0, 2, 1).unsqueeze(3)# batch x k x c x 1
            return ocr_context
...
class SpatialOCR_ASP_Module_Mep(nn.Module):
    def __init__(self, configer, features, hidden_features=256, out_features=512, dilations=(12, 24, 36), num_classes=19, bn_type=None, dropout=0.1):
        super(SpatialOCR_ASP_Module_Mep, self).__init__()
    ...
    def forward(self, x, probs):
        ...
        # mep
        feat2,feat3,feat4,feat5, proj = self.mep_head(feat2,feat3,feat4,feat5)     
        # mep
...
class FCN_Module_Mep(nn.Module):
    def __init__(self, configer, features, hidden_features=256, out_features=512, dilations=(12, 24, 36), num_classes=19, bn_type=None, dropout=0.1):
        super(FCN_Module_Mep, self).__init__()
    ...
    def forward(self, x):
        ...
        # # mep
        # proj = self.mep_head(feat2,feat3,feat4,feat5)     
        # # mep
...
class SpatialOCR_ASP_Module_MLM(nn.Module):
    def __init__(self, configer, features, hidden_features=256, out_features=512, dilations=(12, 24, 36), num_classes=19, bn_type=None, dropout=0.1):
        super(SpatialOCR_ASP_Module_MLM, self).__init__()
        from lib.models.modules.spatial_ocr_block import SpatialOCR_Context
        self.context = nn.Sequential(nn.Conv2d(features, hidden_features, kernel_size=3, padding=1, dilation=1, bias=True),
                                     ModuleHelper.BNReLU(hidden_features, bn_type=bn_type),
                                     SpatialOCR_Context(in_channels=hidden_features,
        ...
        self.proj_bn = nn.Sequential(
            nn.Conv2d(hidden_features * 3, hidden_features, kernel_size=1, padding=0, dilation=1, bias=True),
            ModuleHelper.BNReLU(out_features, bn_type=bn_type)
            )
        self.conv_bn_dropout = nn.Sequential(
        ...
        # mlm
        from lib.models.modules.contrast import MLM_Module
        self.mep_head = MLM_Module(configer, hidden_features)
    ...
    def forward(self, x, probs):
        ...
        # mep
        proj = self.mep_head(feat3,feat4,feat5)     
        # mep

        if isinstance(x, Variable):
            project = torch.cat((feat3, feat4, feat5), 1)
            project = self.proj_bn(project)
            out = torch.cat((feat1, project), 1)
        elif isinstance(x, tuple) or isinstance(x, list):
